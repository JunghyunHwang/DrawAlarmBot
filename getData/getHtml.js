...
exports.getProducts = (startTime) => {
    ...
    function formatReleaseInfo(releaseDayInfo) {
        ...
        return {drawDate, drawStartTime, drawEndTime, announcementTime, purchaseTime};
    }
    ...
    async function getSneakersInfo(drawList) {
        ...
        for (let i = 0; i < drawList.length; i++) {
            ...
            let img_url = $(imgInfo).find('img.image-component').attr('src');

            let drawData = {
            type: type,
            name: name,
            price: price,
            draw_date: drawInfo.drawDate,
            draw_start_time: drawInfo.drawStartTime,
            draw_end_time: drawInfo.drawEndTime,
            announcement_time: drawInfo.announcementTime,
            purchase_time: drawInfo.purchaseTime,
            url: drawList[i].url,
            img_url: img_url
            };
